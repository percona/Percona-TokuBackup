...
static void* doit(void* ignore) {
    ...
    { int r = sf.lock_range(doit_lo, doit_hi);   assert(r==0);  }
    stepa = 1;


    stepb = 1;
    while (!stepc);
    { int r = sf.unlock_range(doit_lo, doit_hi); assert(r==0); }
...
static void thread_test_block(void) {
    ...
    { int r = sf.lock_range(9,12);           assert(r==0); }
    assert(stepb==1); // must have gotten to stepb in the doit function.(
    
    {
        void *result;
        int r = pthread_join(th, &result);
        assert(r==0);
        assert(result==&the_char);
    }
    { int r = sf.unlock_range(9,12);           assert(r==0); }
...
static void thread_test_noblock(void) {
    ...
    { int r = sf.lock_range(12,15);             assert(r==0); }
    ...
    { int r = sf.unlock_range(12,15);           assert(r==0); }
    VALGRIND_HG_ENABLE_CHECKING(&stepa, sizeof(stepa));
    VALGRIND_HG_ENABLE_CHECKING(&stepb, sizeof(stepb));
    VALGRIND_HG_ENABLE_CHECKING(&stepc, sizeof(stepc));
}
...
int test_main(int argc __attribute__((__unused__)), const char *argv[] __attribute__((__unused__))) {
    ...
    {   int r = sf.init();                          assert(r==0); }
    {   int r = sf.lock_range(0, LLONG_MAX);        assert(r==0); }
    assert( sf.lock_range_would_block_unlocked(0, 1));
    assert(!sf.lock_range_would_block_unlocked(0, 0));
    assert( sf.lock_range_would_block_unlocked(10, 100));
    assert(!sf.lock_range_would_block_unlocked(10, 10));
    assert( sf.lock_range_would_block_unlocked(LLONG_MAX-1, LLONG_MAX));
    assert(!sf.lock_range_would_block_unlocked(LLONG_MAX, LLONG_MAX));
    {   int r = sf.unlock_range(0, LLONG_MAX);      assert(r==0); }


    // Test two ranges that are adjacent.
    {   int r = sf.lock_range(10, 20);              assert(r==0); }
    {   int r = sf.lock_range(20, 30);              assert(r==0); }
    assert(!sf.lock_range_would_block_unlocked(0, 10));
    for (int i=10; i<30; i++) {
        for (int j=i+1; j<=30; j++) {
            assert(sf.lock_range_would_block_unlocked(i, j));
        }
    }
    assert(!sf.lock_range_would_block_unlocked(30, LLONG_MAX));
    {   int r = sf.unlock_range(10, 20);            assert(r==0); }
    {   int r = sf.unlock_range(20, 30);            assert(r==0); }

    // test two ranges with a gap in between.
    {   int r = sf.lock_range(10, 20);              assert(r==0); }
    {   int r = sf.lock_range(30, 40);              assert(r==0); }
    for (int i=0; i<50; i++) {
        for (int j=i; j<50; j++) {
            bool expect_block =
    ...
    {   int r = sf.unlock_range(10, 20);            assert(r==0); }
    {   int r = sf.unlock_range(30, 40);            assert(r==0); }
