...
public:
    ...
    ~backup_session();
    int init(const char *source, const char *dest); // returns error code if there was an error setting the directory names.
    ...
    bool is_prefix_of_realpath(const char *absfile);
