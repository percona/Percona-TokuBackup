...
const char *backupdir = "./bench.backup." STRINGIFY(DIRSUF);
...
static void benchmark_setup (void) {
	...
	int l = snprintf(unlink_cmd, sizeof(unlink_cmd), "rm -rf %s", dbdir);
        assert(l<(int)sizeof(unlink_cmd));
	//printf("unlink_cmd=%s\n", unlink_cmd);
	r = system(unlink_cmd);
        CKERR(r);
    ...
    {
	char unlink_cmd[strlen(backupdir) + strlen("rm -rf ") + 1];
	int l = snprintf(unlink_cmd, sizeof(unlink_cmd), "rm -rf %s", backupdir);
        assert(l<(int)sizeof(unlink_cmd));
	printf("unlink_cmd=%s\n", unlink_cmd);
	r = system(unlink_cmd);
        CKERR(r);

        if (strcmp(backupdir, ".") != 0) {
            r = toku_os_mkdir(backupdir,S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
            assert(r == 0);
        }
    }
...
static void* do_backup(void *ignore) {
    sleep(2);
    const char *sources[] = {dbdir};
    const char *dests  [] = {backupdir};
    int r = tokubackup_create_backup(sources, dests, 1,
                                     simple_poll, NULL, simple_error, NULL);
    assert(r==0);
    return ignore;
}
...
static int test_main (int argc, char *const argv[]) {
    ...
    pthread_t pt;
    {
        int r = pthread_create(&pt, NULL, do_backup, NULL);
        assert(r==0);
    }
    gettimeofday(&t1,0);
